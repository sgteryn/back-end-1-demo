...
getSubmit.addEventListener('click', () => {
    axios
        .get('http://localhost:4444/api/inventory')
        .then(res => addToView(res.data))
});

getParamsSubmit.addEventListener('click', () => {
    axios
        .get(`http://localhost:4444/api/inventory/${paramsInput.value}`)
        .then(res => addToView([res.data]))
});

getQuerySubmit.addEventListener('click', () => {
    axios
        .get(`http://localhost:4444/api/inventory?item=${queryInput.value}`)
        .then(res => addToView(res.data))
});
